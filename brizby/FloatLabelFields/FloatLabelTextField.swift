...
@IBDesignable class FloatLabelTextField: UITextField {
	...
	@IBInspectable var titleTextColour:UIColor = UIColor(red: 211, green: 211, blue: 211, alpha: 1.0) {
		didSet {
			if !isFirstResponder() {
	...
	@IBInspectable var titleActiveTextColour:UIColor = UIColor.blueColor() {
		didSet {
			if isFirstResponder() {
	...
	private func maxTopInset()->CGFloat {
		return max(0, floor(bounds.size.height - font.lineHeight - hintYPadding))
	}
