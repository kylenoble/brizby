...
class SignUpViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    var homeCityAnchor:CGFloat = 0.0
    ...
    override func viewDidLoad() {
        ...
        homeCityTextField.delegate = self

        emailAnchor = self.emailTextField.center.y - 18
        passwordAnchor = self.passwordTextField.center.y - 21
        homeCityAnchor = self.homeCityTextField.center.y - 24
        photoAnchor = self.addProfImageButton.center.y - 21


        var fields:[UITextField] = [emailTextField!, passwordTextField!, homeCityTextField!]
        ...
        case .Four:
            ...
            self.homeCityTextField.font = UIFont(name: "Alegreya Sans", size: 18.0)
            for field in fields {
        ...
        homeCityTextField.attributedPlaceholder = NSAttributedString(string: "Home City", attributes: attributesDictionary)
    ...
    func arrangeViewOffsetFromKeyboard()
    {
        ...
        if textFieldLowerPoint.y + kPreferredTextFieldToKeyboardOffset > self.keyboardFrame.origin.y {
            UIView.animateWithDuration(0.2, animations:  {
                ...
                self.homeCityTextField.frame.origin.y -= (targetPointOffset * -1)
    ...
    func textFieldShouldReturn(textField: UITextField) -> Bool {
        ...
        if textField == self.emailTextField {
            ...
            self.homeCityTextField.becomeFirstResponder()
        }
        else {
            textField.resignFirstResponder()
            resetTextFieldPositions()
    ...
    func resetTextFieldPositions() {
        ...
        UIView.animateWithDuration(0.2, animations:  {
            ...
            self.homeCityTextField.center.y = self.homeCityAnchor
    ...
    @IBAction func signupButtonPressed(sender: UIButton) {
        ...
        if let selectedImage =  self.profilePicImage {
            ...
            imageUrl = "https://s3-us-west-1.amazonaws.com/brztst/\(name)"
        ...
        let parameters = [
            "api_v1_user": [
                ...
                "home_city": "\(homeCityTextField.text)",
                "avatar_attributes": [
                    "direct_upload_url": "\(imageUrl)"
                ]
            ]
        ]

        Alamofire.request(APIManager.Router.CreateUser(params: parameters))
            ...
            if let code = myJSON?["state"]["code"] {
                if code == 0 {
                    ...
                    NSUserDefaults.standardUserDefaults().setObject(myJSON?["data"]["id"].string, forKey: kUserId)
                    
                    NSUserDefaults.standardUserDefaults().synchronize()
                    
                    let token = myJSON?["data"]["auth_token"].string
                    let email = myJSON?["data"]["email"].string
                    let id = myJSON?["data"]["id"].int
